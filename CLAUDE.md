...
- SCSS-модули (`*.module.scss` рядом с компонентом) + дизайн-токены в [frontend/src/styles/](frontend/src/styles/) (`_tokens.scss`, `_mixins.scss`, `global.scss`). Темы — через CSS-переменные на `:root` / `.dark` в `global.scss`, без Tailwind
- Axios — HTTP запросы
